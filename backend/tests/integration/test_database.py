...
class TestModels:
    ...
    async def test_user_model_creates_with_required_fields(self, db_session):
        ...
        """
        user = User(email="newuser@example.com", password_hash="hashed123")
        db_session.add(user)
    ...
    @pytest.mark.asyncio
    async def test_task_cascade_deletes_when_user_deleted(self, db_session, test_user, test_task):
    ...
    async def test_audit_log_creates_with_timestamp(self, db_session, test_user):
        ...
        """
        log = AuditLog(user_id=test_user.id, action="CREATE_TASK", result="success")
        db_session.add(log)
        await db_session.commit()
...
class TestTaskCRUD:
    ...
    async def test_create_task_returns_task_with_id(self, db_session, test_user):
        ...
        task = await TaskCRUD.create(
            db_session, {"title": "New task", "priority": 4, "user_id": test_user.id}
        )
    ...
    @pytest.mark.asyncio
    async def test_get_task_by_id_returns_correct_task(self, db_session, test_user, test_task):
        """GIVEN task exists
        WHEN retrieved by ID
        THEN returns correct task
        """
        retrieved = await TaskCRUD.get_by_id(db_session, test_task.id, test_user.id)
    ...
    async def test_get_task_by_id_returns_none_for_wrong_user(
        ...
        """
        retrieved = await TaskCRUD.get_by_id(db_session, test_task.id, test_user_2.id)
    ...
    async def test_list_tasks_by_user_returns_only_user_tasks(
        ...
        await TaskCRUD.create(db_session, {"title": "User 1 task 1", "user_id": test_user.id})
        await TaskCRUD.create(db_session, {"title": "User 1 task 2", "user_id": test_user.id})

        # Create task for user 2
        await TaskCRUD.create(db_session, {"title": "User 2 task", "user_id": test_user_2.id})
    ...
    async def test_list_tasks_filters_by_status(self, db_session, test_user):
        ...
        await TaskCRUD.create(
            db_session, {"title": "Pending task", "status": "pending", "user_id": test_user.id}
        )
        await TaskCRUD.create(
            db_session, {"title": "Completed task", "status": "completed", "user_id": test_user.id}
        )

        pending = await TaskCRUD.list_by_user(db_session, test_user.id, status="pending")

        assert len(pending) == 1
        assert pending[0].title == "Pending task"

    @pytest.mark.asyncio
    async def test_update_task_modifies_specified_fields(self, db_session, test_user, test_task):
        """GIVEN task exists
        ...
        updated = await TaskCRUD.update(
            db_session, test_task.id, test_user.id, {"status": "completed", "priority": 5}
        )
    ...
    @pytest.mark.asyncio
    async def test_update_task_updates_timestamp(self, db_session, test_user, test_task):
        """GIVEN task exists
        WHEN updated
        ...
        updated = await TaskCRUD.update(
            db_session, test_task.id, test_user.id, {"status": "in_progress"}
        )

        assert updated.updated_at > original_updated

    @pytest.mark.asyncio
    async def test_delete_task_removes_from_database(self, db_session, test_user, test_task):
        """GIVEN task exists
        WHEN deleted
    ...
    async def test_get_pending_tasks_excludes_completed(self, db_session, test_user):
        ...
        """
        await TaskCRUD.create(
            db_session, {"title": "Pending", "status": "pending", "user_id": test_user.id}
        )
        await TaskCRUD.create(
            db_session, {"title": "In progress", "status": "in_progress", "user_id": test_user.id}
        )
        await TaskCRUD.create(
            db_session, {"title": "Completed", "status": "completed", "user_id": test_user.id}
        )
    ...
    async def test_get_pending_tasks_excludes_snoozed(self, db_session, test_user):
        ...
        """
        await TaskCRUD.create(
            db_session, {"title": "Active task", "status": "pending", "user_id": test_user.id}
        )
        await TaskCRUD.create(
            db_session,
            {
...
class TestTaskCRUDErrors:
    ...
    async def test_update_nonexistent_task_raises(self, db_session, test_user):
        ...
        with pytest.raises(ValueError, match="not found"):
            await TaskCRUD.update(db_session, fake_id, test_user.id, {"status": "completed"})

    @pytest.mark.asyncio
    async def test_cannot_access_other_users_task(self, db_session, test_user, test_user_2):
        """GIVEN task belongs to user1
        WHEN user2 tries to access
        THEN returns None (multi-tenancy isolation)
        """
        # User 1 creates task
        task = await TaskCRUD.create(db_session, {"title": "User 1 task", "user_id": test_user.id})
